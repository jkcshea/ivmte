...
#' @param link Link function with which to estimate propbability
...
#'     variables. This is required when the target paramter is the
...
#'     object is returned. If the user declares a variable in the
#'     dataset to be used as the propensity score, then a
...
propensity.mst <- function(formula, data, link = "linear", late.Z,
    ...
    if (class_formula(as.formula(fname))) {
        ...
        ## If variable name is provided, check if it is indeed a propensity score
        phat <- data[[fname]]
        names(phat) <- rownames(data)
        
        if (max(phat) > 1 | min(phat) < 0) stop('Provided propensity scores are not between 0 and 1.')
        model <- NULL
        if (hasArg(late.Z)) {
            ...
            if(hasArg(late.X))  late.X <- restring(substitute(late.X), substitute = FALSE)
            ...
            if(max(duplicated(model[, c(late.X, late.Z)]))) {
                stop("Custom propensity score is not well defined: some (X, Z) values are mapped to more than one propensity.")
           }
        }
