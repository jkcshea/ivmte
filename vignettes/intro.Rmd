...
author: "Joshua Ka Chun Shea and Alexander Torgovitsky"
date: "`r Sys.Date()`"
...
@heckmanvytlacil2005e introduced the marginal treatment effect (MTE) to provide a choice-theoretic interpretation to the widely used instrumental variables model of @imbensangrist1994e.
This interpretation can be used to formally extrapolate from the compliers to estimate treatment effects for other subpopulations.

The **ivmte** package provides a flexible set of methods for conducting this extrapolation.
The package uses the moment-based framework developed by @mogstadsantostorgovitsky2018e, which accomodates either point identification or partial identification (bounds), and allows the user to maintain shape constraints.
...
We have written a paper [@sheatorgovitsky2019wp] that discusses both MTE methodology and the usage of **ivmte**, which should be helpful for users unfamiliar with MTE methods.
The survey article by @mogstadtorgovitsky2018aroe provides additional theoretical background for MTE methods, including the moment-based implementation used in this module, as well as a full literature review to previous work.
...
2. **splines2** for specifying models with polynomial splines.
3. A package for solving linear programs. There are three options here:
    i. Gurobi and the Gurobi R package **gurobi**, which can be obtained from <http://www.gurobi.com/index>.
    ...
    CPLEX can be obtained from <https://www.ibm.com/analytics/cplex-optimizer>.
    It requires a software license, which IBM offers at no cost to academic researchers.
    i. The **lpSolveAPI** package, which is free and open-source, and available from CRAN.

We recommend that users start with **lpSolve**, since this is the easiest to install.
However, Gurobi and CPLEX tend to be at least an order of magnitude faster than **lpSolve**, so if one is doing extensive work with **ivmte** it makes sense to invest a few minutes setting up one of these options.
The package tries to automatically choose a solver from those available, with preference being given to Gurobi and CPLEX.
...
We will use a subsample of 50,000 women from the data used in @angristevans1998taer.
The data is included with **ivmte** and has the following structure.

```{r, drawData-ae}
ae <- read.csv("ae-small.csv")
...
* `morekids` indicates whether a woman had more than two children or exactly two children.
    + Note that @angristevans1998taer removed women with fewer than two children when constructing their sample. This is necessary to use `samesex` as an IV.
    + Also note that the treatment variable in `ivmte` must be binary. Methods extending the framework of @mogstadsantostorgovitsky2018e to multivalued treatments have not yet been developed.
...
An important caveat to this reasoning, first discussed by @imbensangrist1994e, is that it applies only to the group of so-called _compliers_, whose would have a third child if and only if their first two children were same sex.
...
Thus, the complier subpopulation is a small and arguably specialized group of individuals.

Is the relationship between fertility and labor supply for these individuals the same as for other groups?
...
The following bit of code generates some simulated data from a simple DGP:
...
simdata <- data.frame(y,d,z,x)
knitr::kable(head(simdata, n = 10))
```
...
1. `data = ae` is the usual reference to the dataframe to be used.
1. `target = "att"` specifies the target parameter to be the average treatment on the treated (ATT).
Ahead, we discuss how to specify different target parameters.
1. `m0` and `m1` are formulas specifying the MTR functions for the untreated and treated arms, respectively.
The symbol `u` in the formula refers to the unobservable latent variable in the selection equation.
We provide further examples ahead of how to specify the functional form.
1. `ivlike` indicates the regression to be run to create moments to which the model is fit. Ahead, we discuss how to specify multiple equations and how to select subsets of moments, or estimate them on a subset of the data.
1. `propensity` provides an estimating equation for the propensity score.
...
Finally, when the audit procedure terminates, it reports bounds on the resulting bounds on the target parameter, which in this case is the average treatment on the treated (`target = att`).
These bounds are the main product of the method.
...
Thus, the following raises an error **Issue 118**
```{r, non.polynomial.u.error, eval = FALSE}
args[["m0"]] <- log(u) + yob
r <- do.call(ivmte, args)
```

Names other than `u` can be used for the selection equation unobservable, but one must remember to pass the option `uname` to indicate the new name. **ISSUE 117**
...
Because we need to parse these formulas for the variable `u`, there are some limitations regarding the use of factor variables.
...
r <- do.call(ivmte, args)
```
However, one can work around this as follows.

**ERROR RELATED To 111***
```{r, factor.ok, eval = FALSE}
...
Constant splines, which have an important role in some of the theory developed by @mogstadsantostorgovitsky2018e, have `degree = 0`.
The vector `knots` indicates the piecewise regions for the spline.
Note that `0` and `1` are always automatically included as `knots`, so that only the internal points need to be specified.
...
In order to produce non-trivial bounds in cases of partial identification, `m0.lb` and `m1.lb` are set to the smallest value of the response variable (as inferred from `ivlike`) that is observed in the data, while `m0.lb` and `m1.lb` are set to the largest value.
...
This difficulty is addressed in the `ivmte` function through an auditing procedure.

The auditing procedure is based on two grids: a relatively small _constraint grid_, and a relatively large _audit grid_.
The MTR functions (and implied MTE function) are made to satisfy the specified shape restrictions on the constraint grid by adding constraints to the linear programs that are solved.
However, these constraints might not ensure that the MTR functions satisfy these constraints "everywhere."
...
If the solution MTRs violate the desired restrictions at some of the points in the audit grid, we add some of these points to the constraint grid and repeat the procedure.
The procedure ends (the audit is passed), when the solution MTRs satisfy the constraints over the entire audit grid.

There are certain parameters that are required for the audit procedure: the initial points in the constraint grid (`initgrid.nu` and `initgrid.nx`), the points in the audit grid (`audit.nu` and `audit.nx`), the maximum number of points that are added from the audit grid to the constraint grid (`audit.add`), and the maximum number of times the audit procedure is repeated before giving up (`audit.max`).
...
Also, if `audit.max` is hit, which should be unlikely given the default settings, one should either adjust the settings or examine the `violations` field of the returned results to see the extent to which the shape restrictions are not met.

### Specifying the Target Parameter

The target parameter is the object the researcher wants to know.
The package has built-in support for conventional target parameters and a rich class of generalized LATEs.
It also has a system that allows users to input their own target parameters.
...
We demonstrate this using the simulated data. **ISSUE 104**

```{r, late, eval = FALSE}
args <- list(data = simdata,
             ivlike =  y ~ d + z + d*z,
             target = "late",
             late.from = c(z = 1),
             late.to = c(z = 3),
...
We can condition on covariates in the definition of the LATE by adding the named list `late.X` as follows. **NOT EVALUATED UNTIL THE ABOVE IS**

```{r, late.cond, eval = FALSE}
args[["late.X"]] = c(x = 2)
r <- do.call(ivmte, args)
args[["late.X"]] = c(x = 8)
r <- do.call(ivmte, args)
...
The package also provides support for generalized LATEs, i.e. LATEs where the intervals of `u` that are being considered may or may not correspond to points in the support of the propensity score.
...
args <- list(data = simdata,
             ...
             propensity = d ~ z + x)
r <- do.call(ivmte, args)
p <- predict(r$propensity$model,
             newdata = data.frame(samesex = c(0,1)),
             type = "response")
args[["genlate.lb"]] <- p[1]
args[["genlate.ub"]] <- p[3]
r <- do.call(ivmte, args) # should coincide with standard LATE 1 -> 3 from above
```
...
```{r, genlate.cond}
args[["late.X"]] <- c(x = 2)
r <- do.call(ivmte, args) # should coincide with above
```
...
Specifying the treated weights works the same way but with `target.knots1` and `target.weight`.
Note that the `target` option is ignored when a set of knots and weights are passed.

Here is an example of using custom weights to replicate the LATE from above.

```{r, custom.weights}
pmodel <- r$propensity$model # returned from the previous run of ivmte
xeval = 1 # x = xeval is the point we condition on
px <- mean(simdata$x == xeval) # probability that x = xeval
z.from = 1
z.to = 3
weight1a  <- function(x) {
    if (x != xeval) {
        ...
        x.to <- data.frame(x = xeval, z = zfrom)
        x.from <- data.frame(x = xeval, z = zto)
        p.to <- predict(pmodel, newdata = x.to, type = "response")
        p.from <- predict(pmodel, newdata = x.from, type = "response")
        return(1 / ((p.to - p.from) * px))
    }
}
weight0a <- function(x1) {
    return(-weight1a(x1))
}
## Define knots (same for treated and control)
knot1a <- function(x) {
    x.from <- data.frame(x = x, z = z.from)
    p.from <- predict(pmodel, newdata = x.from, type = "response")
    return(p.from)
}
knot2a <- function(x) {
    x.to <- data.frame(x = x, z = z.to)
    p.to <- predict.glm(pmodelA, x.to, type = "response")
    return(p.to)
}
```

The knot specification here is the same for both the treated and untreated weights.
It specifies two knots that depend on whether `x == 0`, so that there are four knots total when accounting for 0 and 1, which are always included.
...
The weights in the second region are only non-zero when `x == 0`, in which case they are equal to the inverse of the distance between the second and third knot points.
Thus, the weighting scheme applies equal weight to compliers with `x == 0`, and zero weight to all other others.
As expected, the resulting bounds match those that we computed above using `target == late`.

**ADD A MORE INTERESTING EXAMPLE? Depends on what built-in functionality we end up keeping**
...
Third, one can specify a subset of the data on which the formula is run, which allows one to condition on observables.
...
As indicated by the output, there are 8 moments being fit in this specification.
Three of these moments correspond to the constant and the coefficients on `z` and `x` from the first linear regression of `y` on `z` and `x`.
There are three more from the second regression of `y` on `d` and `x`.
...
The default behavior of `ivmte` is to use all of the moments from equal formula included in `ivlike`.
...
One can tell `imvte` to only include certain moments and not others by passing the `components` option.
This option expects a list of the same length as the list passed to `ivlike`.
Each component of the list is itself a list of the variables names for the coefficients to be included from that formula.
For example, the following includes the constant (intercept) term and the coefficient on `z` from the first formula, all moments from the second formula, and just the coefficient on `d` from the third formula.

```{r, ivlike.components}
args[["components"]] <- l(c(intercept, x), , d)
r <- do.call(ivmte, args)
```
Note that `intercept` is a reserved word.
...
For example, the following would run the first regression on the entire subsample, the second regression only on those observations that have `z` equal to 1 or 3, and the third (TSLS) formula only on observations with `x` smaller than 5.

```{r, ivlike.subsets}
args[["subset"]] <- l(1 == 1, z %in% c(1,3), x <= 5)
r <- do.call(ivmte, args)
```
...
The default is to estimate the formula as a logit model via the `glm` command, but this can be change this to `"linear"` or `"probit"` using the `link` option.
...
If one believes the model is in fact point identified, then `point == TRUE` can be included.
Typically, this will be the case when `ivlike`, `components` and `subset` are set such that they have more (non-redundant) components than there are free parameters in `m0` and `m1`.
Bounds in this case will typically shrink to a point.

**ISSUE 119**

```{r, point.id, eval = FALSE}
args <- list(data = ae,
             target = "att",
             m0 = ~ u,
             m1 = ~ u,
...
The default when `point = TRUE` is to use two-step GMM via the **gmm** package, however the identitiy weighting can also be used by passing `point.eyeweight = TRUE`.

**ISSUE 119**

```{r, point.id.eyeweight, eval = FALSE}
args[["point.eyeweight"]] <- TRUE
r <- do.call(ivmte, args)
```
...
**ISSUE 120 -- very strange results here**
...
The default is to set `bootstraps.m` to the the sample size of the observed data with `bootstraps.replace == TRUE`.
This corresponds to the usual nonparametric bootstrap.
Choosing `bootstraps.m` to be smaller than the sample size and settings `bootstraps.replace` to be `TRUE` or `FALSE` corresponds to the "m out of n" bootstrap or subsampling, respectively.
Regardless of these settings, two types of confidence regions are constructed following the terminology of @andrewshan2009ej; see @sheatorgovitsky2019wp for more detail, references, and important caveats to the procedures.
...
**ISSUE 119 PREVENTS RUNNING THE SECOND PART**

```{r, misspecification.test, eval = FALSE}
args <- list(data = simdata,
             ivlike =  y ~ d + factor(z),
             target = "ate",
             m0 = ~ u,
...
For help, feature requests and bug reports, please post an issue on our GitHub repository.
